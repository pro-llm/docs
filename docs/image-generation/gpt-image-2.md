...
五个模型名,**后端是同一个 GPT Image 2**,区别只在 `quality` 的默认值:
...
| `gpt-image-2-c` | 由你指定(不传则 `low`) |
| `gpt-image-2` | 同上 |

**名字只决定 `quality` 的默认值,不影响尺寸。** 请求里显式传了 `quality`,以你传的为准。

## quality

取值 `low` / `medium` / `high`,与官方一致。它控制**渲染精细度**,不影响输出尺寸。

不传则用模型名自带的默认值;用 `gpt-image-2` 或 `-c` 且不传时为 `low`。
