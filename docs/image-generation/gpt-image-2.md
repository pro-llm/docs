...
| 数量 | 1 – 16 张;一张都没有返回 400(`"param": "image"`),超过 16 张返回 400 |
| 格式 | PNG / JPEG / WebP |
...
| `mask`(图生图) | 接受,但**不保证只改遮罩区域**,遮罩外也可能被重绘;不要依赖它做局部重绘 |
...
| `low` | 15 – 45 s | ~75 s |
| `medium` | 40 – 80 s | ~90 s |
| `high` | 120 – 200 s | ~250 s |

以上是服务端出图时间,图片传输另计(3840×2160 的 PNG 约 10 MB)。
耗时受上游负载影响明显,高峰期会整体拉长,**`high` 档尤其慢,请务必给足 600 秒**。
