...
| 档位 | 常见耗时(首字节) | 实测长尾 |
| --- | --- | --- |
| Gemini 1K | 15 – 45 s | ~90 s |
| Gemini 2K | 20 – 60 s | ~90 s |
| Gemini 4K | 40 – 80 s | ~120 s |
| GPT Image 2 `low` | 15 – 45 s | ~75 s |
| GPT Image 2 `medium` | 40 – 80 s | ~90 s |
| GPT Image 2 `high` | 120 – 200 s | ~250 s |

以上是服务端出图时间,图片传输另计。高峰期会整体拉长。
**Gemini 请把客户端超时设到 300 秒(4K 弱网建议 600 秒),GPT Image 2 设到 600 秒。**
