...
!!! warning "客户端超时给足:Gemini 300 秒,GPT Image 2 600 秒"
    生图是长耗时操作,Gemini 1K 常见 15–45 秒、4K 可到 120 秒,GPT Image 2 `high` 档常见 120–200 秒。默认的 30 / 60 秒超时几乎必然中途断开。

!!! warning "不要自己加激进重试"
    服务内部已有自动重试与账号轮换。你收到的错误都是重试之后仍未成功的终态结果。
    429 / 500 / 503 退避几秒后**重试一次**即可,循环重试只会加重排队;400 / 451 是终态,不要重试。
    报障时附上响应头 `X-Oneapi-Request-Id`。
